...
public class FileUploadControllerMaster {
    ...
    @GetMapping("/file/{type}/{patientInfo}/{visitId}/{documentType}/{filename:.+}")
    @ResponseBody
    public ResponseEntity<Resource> serveFile(@PathVariable String type,
                                              @PathVariable String patientInfo,
                                              @PathVariable String visitId,
                                              @PathVariable String documentType,
                                              @PathVariable String filename, @RequestHeader(HttpHeaders.AUTHORIZATION) String token) {
        String finalFilePath =  patientInfo + "/" + visitId + "/" + documentType + "/" + filename;
        Resource file = storageService.loadAsResource(finalFilePath, type);
